...
from creditops.api.underwriting import UNDERWRITER_ROLE
...
class FakeOrchestrationRepository:
    ...
    ``load_snapshot`` reflects the gates written via ``ensure_gate`` -- the
    ``/submit`` review-gate precondition reads them back."""
    ...
    async def record_proposal(self, **kwargs: object) -> None:
        raise AssertionError("not used by the underwriting API")
...
# ---------------------------------------------------------------------------
# Stage-4/5 human gate writes: POST /review and POST /submit
# ---------------------------------------------------------------------------
...
def _build_write_client(
    signing_key: rsa.RSAPrivateKey,
    *,
    repository: FakeUnderwritingReadRepository,
...
def test_underwriter_review_satisfies_gate_and_reticks(
    ...
    response = client.post(
        f"/api/v1/cases/{CASE_ID}/underwriting/review",
        json={"assessmentId": str(ASSESSMENT_ID), "rationale": "Da xem xet dat yeu cau."},
        headers={"Authorization": f"Bearer {token(signing_key, roles=[UNDERWRITER_ROLE])}"},
    )

    assert response.status_code == 200
    body = response.json()
    assert body["gateType"] == GateType.HG_UNDERWRITING_ASSESSMENT_REVIEWED.value
    assert body["status"] == GateStatus.SATISFIED.value
    assert body["assessmentId"] == str(ASSESSMENT_ID)
    assert body["dispositionRef"] == f"underwriting-assessment:{ASSESSMENT_ID}"

    assert len(orchestration.ensure_gate_calls) == 1
    call = orchestration.ensure_gate_calls[0]
    assert call["gate_type"] == GateType.HG_UNDERWRITING_ASSESSMENT_REVIEWED
    assert call["status"] == GateStatus.SATISFIED
    assert call["satisfied_by_actor_id"] == OFFICER_A
    assert call["disposition_ref"] == f"underwriting-assessment:{ASSESSMENT_ID}"
    # Retick: a fresh ORCHESTRATOR_PLAN task is created, outboxed and dispatched.
    plan_tasks = [
    ...
    # The review was audited.
    assert any(
        getattr(e, "event_type", None) == "UNDERWRITING_ASSESSMENT_REVIEWED"
...
def test_review_with_no_assessment_yet_is_404(signing_key: rsa.RSAPrivateKey) -> None:
    repository = FakeUnderwritingReadRepository(has_assessment=False)
    orchestration = FakeOrchestrationRepository()
    client = _build_write_client(
        signing_key, repository=repository, orchestration_repository=orchestration
    )
    ...
    response = client.post(
        f"/api/v1/cases/{CASE_ID}/underwriting/review",
        json={"assessmentId": str(ASSESSMENT_ID), "rationale": "Chua co ban phan tich."},
        headers={"Authorization": f"Bearer {token(signing_key, roles=[UNDERWRITER_ROLE])}"},
    )
...
def test_review_rejects_non_underwriter_role(signing_key: rsa.RSAPrivateKey) -> None:
    repository = FakeUnderwritingReadRepository()
    orchestration = FakeOrchestrationRepository()
    client = _build_write_client(
        signing_key, repository=repository, orchestration_repository=orchestration
...
def test_review_by_unassigned_underwriter_is_indistinguishable_404(
    signing_key: rsa.RSAPrivateKey,
) -> None:
    repository = FakeUnderwritingReadRepository()
    orchestration = FakeOrchestrationRepository()
    ...
    response = client.post(
        ...
        headers={
            "Authorization": (
                f"Bearer {token(signing_key, subject=uuid4(), roles=[UNDERWRITER_ROLE])}"
            )
        },
    )
...
def test_submit_before_review_is_409_review_required_first(
    signing_key: rsa.RSAPrivateKey,
) -> None:
    repository = FakeUnderwritingReadRepository()
    orchestration = FakeOrchestrationRepository()
    ...
    response = client.post(
        f"/api/v1/cases/{CASE_ID}/underwriting/submit",
        json={"assessmentId": str(ASSESSMENT_ID), "rationale": "Trinh phuong an."},
        headers={"Authorization": f"Bearer {token(signing_key, roles=[UNDERWRITER_ROLE])}"},
    )

    assert response.status_code == 409
    assert response.json()["code"] == "REVIEW_REQUIRED_FIRST"
    # No submission gate was written.
    assert orchestration.ensure_gate_calls == []


def test_review_then_submit_satisfies_submission_gate_and_reticks(
    signing_key: rsa.RSAPrivateKey,
) -> None:
    repository = FakeUnderwritingReadRepository()
    orchestration = FakeOrchestrationRepository()
    ...
    review = client.post(
        f"/api/v1/cases/{CASE_ID}/underwriting/review",
        json={"assessmentId": str(ASSESSMENT_ID), "rationale": "Da xem xet."},
        headers=headers,
    )
    ...
    body = submit.json()
    assert body["gateType"] == GateType.HG_MAKER_SUBMISSION_CONFIRMED.value
    assert body["status"] == GateStatus.SATISFIED.value
    assert body["dispositionRef"] == f"maker-submission:{ASSESSMENT_ID}"

    submission_calls = [
        c
        for c in orchestration.ensure_gate_calls
        if c["gate_type"] is GateType.HG_MAKER_SUBMISSION_CONFIRMED
    ]
    assert len(submission_calls) == 1
    assert submission_calls[0]["status"] == GateStatus.SATISFIED
    assert submission_calls[0]["disposition_ref"] == f"maker-submission:{ASSESSMENT_ID}"
    # Both gate satisfactions reticked the orchestrator.
    plan_tasks = [
        t for t in orchestration.created_tasks if t["task_type"] is TaskType.ORCHESTRATOR_PLAN
    ]
    assert len(plan_tasks) >= 1
...
def test_submit_rejects_non_underwriter_role(signing_key: rsa.RSAPrivateKey) -> None:
    repository = FakeUnderwritingReadRepository()
    orchestration = FakeOrchestrationRepository()
    client = _build_write_client(
        signing_key, repository=repository, orchestration_repository=orchestration
