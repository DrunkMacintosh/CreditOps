...
## Partial synthetic configuration for roles, dispositions, and gates (2026-07-18)

Recorded while implementing the synthetic prototype configuration described in the 2026-07-18 rows of the [Decision Log](DECISION_LOG.md). Each item below gives a code-level placeholder to the underlying question so the prototype can run; none of them is resolved, no official SHB source has been supplied for any of them, and none may be treated as a requirement or banking rule.

- **Pointer:** "Which human roles are responsible for underwriting, independent risk review, legal review, collateral review, operations, and approval?" (Business and banking, above) is now partially addressed by a closed, synthetic ten-role `case_assignments.case_role` set — `INTAKE_OFFICER, UNDERWRITER, LEGAL_REVIEWER, RISK_REVIEWER, OPS_OFFICER, OPS_CHECKER, ACTION_AUTHORIZER, MONITORING_OFFICER, COLLECTIONS_OFFICER, AUDITOR` (`supabase/migrations/202607180008_case_assignment_roles.sql`). The migration's own comment states this has no official SHB role mapping, RACI, or delegation-of-authority basis. Remains OPEN.
- **Pointer:** "How are policy exceptions identified, escalated, decided, and recorded?" (Policy and evidence, above) is now partially addressed by two synthetic disposition taxonomies: the Independent Risk Review continue-set `NOTED` / `ACCEPTED_RISK` versus the non-continuing `MAKER_MUST_REVISE` / `ESCALATED` (`services/api/src/creditops/application/orchestration/gates.py`, `G3_CONTINUE_DISPOSITION_TYPES`), and the pre-Risk gap-request batch disposition set `APPROVED_ALL` / `APPROVED_WITH_CHANGES` / `REJECTED` / `NO_OUTBOUND_REQUESTS` (`services/api/src/creditops/domain/gap_request_batches.py`). Both are labelled synthetic at the source and carry no official SHB basis. Remains OPEN.
- **Pointer:** "What are the official case states, allowed transitions, service-level expectations, and escalation paths?" (Workflow and integration, above) is now partially addressed by the prototype's own synthetic gate vocabulary — `G1_INTAKE_COMPLETE`, `G2_GAP_REQUEST_APPROVAL`, `G3_RISK_DISPOSITION`, `G4_OPS_AUTHORIZATION` (`services/api/src/creditops/application/orchestration/gates.py`). These are working names for this codebase's own state machine, not official SHB case states, transitions, or SLAs. Remains OPEN.

## Frontend contract-pending items narrowing (2026-07-18)

Recorded against the "Frontend intake review contracts (2026-07-18)" section above. Backend endpoints now exist implementing three of that section's four named Task 9 paths: the gap-request batch assemble/read/disposition routes (`services/api/src/creditops/api/gap_requests.py`), the explicit intake-completion and current-handoff routes (`services/api/src/creditops/api/intake.py`), and the cursor-paginated audit-events route (`services/api/src/creditops/api/audit.py`).

This narrows, but does not close, the first bullet of that section ("Task 9 endpoint paths ... are unpublished; the frontend renders contract-pending states"): the routes are implemented and internally documented, but no canonical OpenAPI has been published for them, and the frontend has not yet been wired to them — `apps/web/components/gaps/gap-list.tsx`, `apps/web/components/handoff/handoff-summary.tsx`, and `apps/web/components/audit/audit-timeline.tsx` still render their own `*_CONTRACT_PENDING_TEXT` fallback rather than calling these routes. The section's other four bullets (the `rationale` field on CORRECTED dispositions, the conflict wire shape, the document preview-URL contract, and the extraction field-key vocabulary) are unaffected: `services/api/src/creditops/domain/evidence.py`'s `FactConfirmation` still carries no `rationale` field, and none of today's changes touch the remaining three contracts.
