...
**CURRENT STATUS:** A local walking-skeleton prototype is implemented: domain contracts, migrations, orchestration state machine and task/lease/checkpoint logic, the upload-intent flow, pure document-processing stages, an FPT capability gateway, Underwriting/Legal/Risk/Credit-Operations processors, and the related Vietnamese frontend workbenches. It is not a deployed or end-to-end runnable banking system: the approved target architecture uses Vercel for the Vietnamese frontend, Supabase for durable shared state, queues, retrieval metadata, and object storage, Google Cloud Run for the FastAPI API and asynchronous workers, and FPT AI Factory only for managed model inference, but none of these services has been provisioned or deployed, the worker refuses to run without injected real dependencies, and no live FPT smoke test has recorded evidence. Final model selection remains benchmark-gated (routes fail closed without a committed benchmark-pass record), and no official SHB policy corpus, checklist, workflow configuration, or API sandbox is available. The master design for the full 14-stage lifecycle is [`docs/superpowers/specs/2026-07-18-full-credit-lifecycle-agent-workflow-design.md`](docs/superpowers/specs/2026-07-18-full-credit-lifecycle-agent-workflow-design.md).
